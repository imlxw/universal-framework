...
import com.microservices.db.dialect.IMicroservicesModelDialect;
...
public class MicroservicesModel<M extends MicroservicesModel<M>> extends Model<M> {

	public static final String AUTO_COPY_MODEL = "_auto_copy_model_";

	private static final String COLUMN_CREATED = MicroservicesModelConfig.getConfig().getColumnCreated();
	private static final String COLUMN_MODIFIED = MicroservicesModelConfig.getConfig().getColumnModified();
	...
	/**
	 * 在 RPC 传输的时候，通过 Controller 传入到Service 不同的序列化方案 可能导致 getModifyFlag 并未设置，可能造成无法保存到数据库 因此需要 通过这个方法 拷贝数据库对于字段，然后再进行更新或保存
	 *
	 * @return
	 */
	public M copyModel() {
	...
	private IMicroservicesModelDialect getDialect() {
		return (IMicroservicesModelDialect) _getConfig().getDialect();
	}
	...
	public Page<M> paginateByColumns(int pageNumber, int pageSize, List<Column> columns, String orderBy) {
		...
		return params.isEmpty() ? paginate(pageNumber, pageSize, selectPartSql, fromPartSql) : paginate(pageNumber, pageSize, selectPartSql, fromPartSql, params.toArray());
	}
	...
	protected Table _getTable(boolean validateNull) {
		if (table == null) {
			...
			if (table == null && validateNull) {
				throw new MicroservicesException(String.format("class %s can not mapping to database table, maybe cannot connection to database or not use correct datasource, "
						+ "please check microservices.properties or correct config @Table(datasource=xxx) if you use multi datasource.", _getUsefulClass().getName()));
			}
		}
		return table;
	}
	...
	protected String _getPrimaryKey() {
		...
		MicroservicesAssert.assertTrue(primaryKey != null, String.format("get PrimaryKey is error in[%s]", getClass()));
		return primaryKey;
	}
	...
	public boolean equals(Object o) {
		...
		if (!(o instanceof MicroservicesModel)) {
			return false;
		}

		Object id = ((MicroservicesModel) o).get(_getPrimaryKey());
		if (id == null) {
			return false;
		}
